...
class ANPA_Socios_Area_REST {
	...
	public static function register_routes(): void {
		...
		/**
		 * GET /area/master/init-status — check whether master init has been completed.
		 *
		 * Requires a valid area session. For master callers when the system has
		 * NOT been initialized, returns the full init status including a generated
		 * banking passphrase. For non-masters or already-initialized, returns
		 * the bare initialized flag.
		 *
		 * @since 1.21.0
		 */
		register_rest_route(
			self::REST_NAMESPACE,
			'/area/master/init-status',
			array(
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => array( __CLASS__, 'handle_master_init_status' ),
				'permission_callback' => array( __CLASS__, 'permission_area_session' ),
			)
		);

		/**
		 * POST /area/master/init — one-time master initialization.
		 *
		 * Requires master session + CSRF (skips admin-password gate because
		 * the admin password does not exist yet). Generates the banking keypair,
		 * wraps the secret key with the provided passphrase, sets the admin
		 * password, and marks the system as initialized. Fails with 400 if
		 * already initialized.
		 *
		 * @since 1.21.0
		 */
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'handle_master_init' ),
				'permission_callback' => array( 'ANPA_Socios_Admin_Shared', 'permission_master_init' ),
				'args'                => array(
					'passphrase' => array(
						'required'          => true,
						'type'              => 'string',
		...
		/**
		 * POST /area/me/admin-auth — verify the admin panel password.
		 *
		 * Requires a valid area session. On success, marks the current session
		 * as admin-authorized so the caller can use master-gated admin endpoints.
		 *
		 * @since 1.21.0
		 */
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( __CLASS__, 'handle_admin_auth' ),
				'permission_callback' => array( __CLASS__, 'permission_area_session' ),
				'args'                => array(
		...
		/**
		 * PUT /area/me/admin-password — change the admin panel password.
		 *
		 * Master-only (full master gate with admin-password check). Validates
		 * the new password against the password policy and updates the stored hash.
		 *
		 * @since 1.21.0
		 */
		register_rest_route(
			self::REST_NAMESPACE,
			'/area/me/admin-password',
			array(
				'methods'             => WP_REST_Server::EDITABLE,
				'callback'            => array( __CLASS__, 'handle_admin_password_change' ),
	...
	/**
	 * GET /area/master/init-status — check master init state.
	 *
	 * @since  1.21.0
	 * @param  WP_REST_Request $request Incoming request.
	 * @return WP_REST_Response
	 */
	public static function handle_master_init_status( WP_REST_Request $request ) {
		$profile = self::request_profile( $request );
		if ( null === $profile ) {
			return self::invalid_session_error();
		}

		$initialized = ANPA_Socios_Master_Auth::is_initialized();
		$is_master   = 'master' === (string) ( $profile['rol'] ?? '' );

		$response = array(
			'initialized'        => $initialized,
			'admin_password_set' => ANPA_Socios_Master_Auth::admin_password_exists(),
		);

		if ( $is_master && ! $initialized ) {
			$response['aviso_master']           = '⚠️ Aínda non se inicializou a base de datos da ANPA. Como administrador/a, tes que xerar as claves de cifrado bancario antes de poder xestionar socios/as. Garda o contrasinal que xeres nun lugar seguro.';
			$response['generated_passphrase']   = ANPA_Socios_Master_Auth::generate_banking_passphrase();
		}
	...
	/**
	 * POST /area/master/init — one-time init of banking keys + admin password.
	 *
	 * @since  1.21.0
	 * @param  WP_REST_Request $request Incoming request.
	...
	public static function handle_master_init( WP_REST_Request $request ) {
		...
		ANPA_Socios_Master_Auth::set_admin_password( $passphrase );
	...
	/**
	 * POST /area/me/admin-auth — verify the admin panel password and mark session.
	 *
	 * @since  1.21.0
	 * @param  WP_REST_Request $request Incoming request.
	 * @return WP_REST_Response|WP_Error
	 */
	public static function handle_admin_auth( WP_REST_Request $request ) {
		$profile = self::request_profile( $request );
		if ( null === $profile ) {
			return self::invalid_session_error();
		}
		...
		if ( '' !== $area_token ) {
			ANPA_Socios_Master_Auth::mark_admin_session_authorized( $area_token );
		}
	 ...
	 * @return WP_REST_Response|WP_Error
	 */
	public static function handle_admin_password_change( WP_REST_Request $request ) {
		$profile = self::request_profile( $request );
		if ( null === $profile ) {
			return self::invalid_session_error();
		}
