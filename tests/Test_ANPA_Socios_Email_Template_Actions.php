...
final class Test_ANPA_Socios_Email_Template_Actions extends TestCase {
	...
	/**
	 * The observable action contract is pinned in its own golden, generated by
	 * `ANPA_ACTIONS_GOLDEN_CAPTURE=1` and never hand-edited.
	 *
	 * Without a recorded value, the digest only proved its own shape: any rewording would have
	 * produced a different-but-equally-valid hash and nothing would have failed. The negative
	 * controls above show that a visible change MOVES the digest; this is what makes the move
	 * visible to a human.
	 */
	public function test_the_actions_fingerprint_matches_its_golden(): void {
		$fingerprint = ANPA_Socios_Email_Template_Actions::fingerprint();
		$path        = $this->golden_path();

		if ( '1' === (string) getenv( 'ANPA_ACTIONS_GOLDEN_CAPTURE' ) ) {
			file_put_contents( $path, $fingerprint . "\n" );
			$this->assertFileExists( $path );
			return;
		}

		$this->assertFileExists( $path, 'capture it with ANPA_ACTIONS_GOLDEN_CAPTURE=1' );
		$this->assertSame(
			trim( (string) file_get_contents( $path ) ),
			$fingerprint,
			'the pending-action contract changed; if that was intended, recapture the golden and say what changed for the reader'
		);
	}

	public function test_the_actions_golden_records_its_scheme(): void {
		if ( ! is_readable( $this->golden_path() ) ) {
			$this->markTestSkipped( 'actions golden not captured yet.' );
		}

		$this->assertStringStartsWith(
			ANPA_Socios_Email_Template_Actions::FINGERPRINT_SCHEME . ':',
			trim( (string) file_get_contents( $this->golden_path() ) )
		);
	}

	/** @return string Absolute path of the actions golden. */
	private function golden_path(): string {
		return __DIR__ . '/golden/template_actions.sha256';
	}
