...
final class Test_ANPA_Socios_Matricula_Gate extends TestCase {
	...
	public function test_admin_never_takes_the_flag_from_forms_or_bodies(): void {
		...
		// 1.68.0: the switch moved to Xestión → Matrículas (REST); Axustes only links there.
		$this->assertStringContainsString( 'page=anpa-socios-management&section=matriculas', $settings );
		$trimestres = $this->src( 'includes/class-anpa-socios-admin-trimestres-handler.php' );
		$this->assertStringContainsString( 'ANPA_Socios_Matricula_Gate_Repo::sincronizar_flag( $curso )', $trimestres );
		$this->assertStringContainsString( "'/trimestres/ventana'", $trimestres );
	...
	public function test_migration_1_41_0_keeps_open_courses_open_and_is_wired(): void {
		...
		$this->assertStringContainsString( "const DB_VERSION = '1.44.0'", $db );
		...
		$this->assertStringContainsString( "define( 'ANPA_SOCIOS_DB_VERSION', '1.44.0' )", $this->src( 'anpa-socios.php' ) );
	}
}
